...
unweighted_county_table <- survey_data %>%
  mutate(age_gender = paste(gender, age_group, sep = " , ")) %>%
  select(county_name, age_gender) %>%
  tbl_summary(
    by = age_gender,
  ...
  add_overall() %>%
  modify_header(label ~ "**Characteristic**") %>%
  modify_spanning_header(-c("label", "stat_0") ~ "**Gender and age**") %>%
  as_gt() %>%
  tab_style(
    style = list(cell_borders(sides = "all", color = "gray", weight = px(1)),
                 cell_text(weight = "bold", color = "black")),
