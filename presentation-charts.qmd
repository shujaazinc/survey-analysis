...
select(variable_title, valid.prc, frq) |>
...
tab_options(table.width = pct(70), data_row.padding = px(5))
...
data <- data.frame(
  Statement = c(
    "I trust Shujaaz", 
    "I am proud to be a Shujaaz fan", 
    "Shujaaz gives me inspiring ideas", 
    "I use something I’ve learned from Shujaaz in my life", 
    "I am transforming the life of others by promoting Shujaaz"
  ),
  Percent = c(50, 59, 56, 21, 52)
)

# Add a dummy xend column for the dumbbell plot (for simplicity, we'll use the same values)
data$xend <- data$Percent

# Plot
ggplot(data, aes(x = 0, xend = Percent, y = reorder(Statement, Percent))) +
  geom_dumbbell(color = "skyblue", size = 1.5, dot_guide = TRUE, dot_guide_size = 0.25) +
  geom_text(aes(label = paste0(Percent, "%"), x = Percent), color = "black", vjust = -1, size = 3) +
  theme_minimal() +
  labs(
    title = "Shujaaz Fan Survey Results",
    subtitle = "Key findings: Only 2 use what they have learned from Shujaaz in their daily life",
    x = "Percentage",
    y = "Survey Statements"
  ) +
  theme(
