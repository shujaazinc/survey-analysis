...
manage_money <- summarize_by_persona(survey_data, 
"T_F1_1", 
"I know how to manage the little money I have to take care of my needs") |>
mutate(label = val) |>
select(indicator, label, persona, engagement, valid.prc, frq ) |>
rename(Percent = valid.prc, Number = frq) |>
filter(!is.na(label)) |>
mutate(label = case_when(label == 1 ~ "Yes",
label == 2 ~ "No",
TRUE ~ NA_character_))

manage_money |>
filter(label == "Yes") |>
select(-c(label, Number, indicator))|>
mutate(engagement = case_when(engagement == "All" ~ "Overall",
engagement == "Yes" ~ "User",
engagement == "No" ~ "Non user",
TRUE ~ NA_character_)) |>
...
# Define a function to perform the proportion test for a given persona
perform_prop_test <- function(persona_value, survey_design) {
  # Filter data for the specific persona and drop NAs
  persona_data <- subset(survey_design, persona == persona_value) %>%
    drop_na(engagement, money_mngt)
  
  # Fit a logistic regression model
  fit <- svyglm(money_mngt ~ engagement, design = persona_data, family = quasibinomial())
  
  # Perform a Wald test to check if the proportion of users differs between the levels of engagement
  wald_test <- regTermTest(fit, "engagement")
...
# Get the unique personas
unique_personas <- survey_data %>%
  distinct(persona) %>%
  pull(persona)

# Perform the test for each persona and store the results in a list
prop_tests <- map(unique_personas, ~ perform_prop_test(.x, survey_design))

# Name the list elements with the corresponding persona values
names(prop_tests) <- unique_personas
